...
const adultDownBtn = document.querySelector(".count-wrap.adult .btn-down"); //성인 숫자 올리기 버튼
const adultUpBtn = document.querySelector(".count-wrap.adult .btn-up"); // 성인 숫자 내리기 버튼
let adultNum = document.querySelector(".count-wrap.adult .num"); // 성인 숫자 
let adultNumVal = parseInt(adultNum.textContent); // 성인 숫자 
const childrenDownBtn = document.querySelector(".count-wrap.children .btn-down"); // 어린이 숫자 올리기 버튼
const childrenUpBtn = document.querySelector(".count-wrap.children .btn-up"); // 어린이 숫자 내리기 버튼
let childrenNum = document.querySelector(".count-wrap.children .num"); // 어린이 숫자 
let childrenNumVal = parseInt(childrenNum.textContent); // 어린이 숫자 
 


numDown(adultDownBtn,adultNumVal,adultNum);
numDown(childrenDownBtn,childrenNumVal,childrenNum);

numUp(adultUpBtn,adultNumVal,adultNum);
numUp(childrenUpBtn,childrenNumVal,childrenNum);

function numDown(btn,num,numTag){
...
// adultNumVal = adultNumVal;
function numUp(btn,num,numTag){
    btn.addEventListener("click",function(){
        if(num == 4){
            num = 4;
        }
